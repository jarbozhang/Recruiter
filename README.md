...
系统采用 **API 拦截优先 + DOM 解析兜底** 的双层策略：

- **API 拦截**（Playwright）：通过 `page.on("response")` 拦截 Boss直聘的内部 API（如 `getBossFriendListV2`），直接获取结构化 JSON 数据。一次请求可获取 60+ 候选人及 30+ 字段（姓名、学历、期望薪资、最后消息等）。
- **DOM 解析**（通用兜底）：当 API 拦截不可用时（如使用 Selenium driver），退化为 JS `querySelectorAll` 从页面 DOM 提取数据。

降级逻辑：检查 driver 是否支持 `intercept_response` → 支持则注册监听并 reload 触发 API → 10s 内未捕获则自动退化到 DOM 模式。
